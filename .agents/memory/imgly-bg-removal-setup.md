...
- In production the process runs from the workspace root, so imgly's default relative `node_modules/@imgly/.../resources.json` lookup fails (ENOENT). Must pass an explicit `publicPath` (file:// URL) resolved via `createRequire(import.meta.url).resolve(...)` — cwd-independent.
