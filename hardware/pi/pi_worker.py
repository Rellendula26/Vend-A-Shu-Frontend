...
SERVO_SCRIPT_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "servo_motor.py"
)
BIN_TO_MOTOR_GPIO = {
    1: 24,
    2: 23,
...
def parse_bin_from_action(action: str) -> int:
    ...
    if action.startswith("dispense_bin_"):
        ...
        if bin_value.isdigit():
            ...
            if parsed_bin in BIN_TO_MOTOR_GPIO:
                return parsed_bin
    raise ValueError(
...
PI_HARDWARE_DRY_RUN = os.getenv("PI_HARDWARE_DRY_RUN", "").lower() in (
    "1",
    "true",
    "yes",
)
...
def run_motor_for_bin(bin_number: int) -> None:
    if PI_HARDWARE_DRY_RUN:
        logging.info(
            "[DRY RUN] Skipping servo subprocess for bin %s", bin_number
        )
        return
    motor_gpio_pin = BIN_TO_MOTOR_GPIO[bin_number]
    led_left_pin, led_right_pin = BIN_TO_LED_GPIOS[bin_number]
    command = [
        "python3",
        SERVO_SCRIPT_PATH,
        "--motor-pin",
